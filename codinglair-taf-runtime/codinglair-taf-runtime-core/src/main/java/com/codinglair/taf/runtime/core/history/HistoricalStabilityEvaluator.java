...
public final class HistoricalStabilityEvaluator {
  ...
  public StabilityStatus evaluate(
      List<ExecutionAttemptSummary> history, int minimumSamples, CompatibilityScope compatibility) {
    if (minimumSamples < 2)
      throw new IllegalArgumentException("minimumSamples must be at least two");
    var completed =
        history.stream()
            .filter(compatibility::includes)
            .filter(
                item ->
                    item.outcome() == ExecutionAttemptSummary.Outcome.PASSED
    ...
    boolean passed =
        completed.stream()
            .anyMatch(item -> item.outcome() == ExecutionAttemptSummary.Outcome.PASSED);
    boolean failed =
        completed.stream()
            .anyMatch(item -> item.outcome() == ExecutionAttemptSummary.Outcome.FAILED);
  ...
  public record CompatibilityScope(
      java.util.Set<String> buildIds, java.util.Set<String> environmentIds) {
    public CompatibilityScope {
      buildIds = java.util.Set.copyOf(buildIds);
      environmentIds = java.util.Set.copyOf(environmentIds);
    }

    public static CompatibilityScope all() {
      return new CompatibilityScope(java.util.Set.of(), java.util.Set.of());
    }
  ...
  public boolean recurring(
      List<ExecutionAttemptSummary> history, String signature, int minimumOccurrences) {
    if (minimumOccurrences < 2)
      throw new IllegalArgumentException("minimumOccurrences must be at least two");
    return history.stream()
            .filter(item -> item.signature() != null)
            .filter(item -> item.signature().value().equals(signature))
            .limit(minimumOccurrences)
            .count()
        >= minimumOccurrences;
  }
}
