...
class HistoricalStabilityEvaluatorTest {
  ...
  void passAndFailureAreFlakyWhileFailureOnlyIsStableRecurring() {
    ...
    assertThat(evaluator.evaluate(List.of(failedOne), 2))
        .isEqualTo(StabilityStatus.INSUFFICIENT_HISTORY);
    assertThat(evaluator.evaluate(List.of(failedOne, failedTwo), 2))
        .isEqualTo(StabilityStatus.STABLE);
    assertThat(evaluator.recurring(List.of(failedOne, failedTwo), failedOne.signature().value(), 2))
        .isTrue();
    assertThat(evaluator.evaluate(List.of(failedOne, passed), 2))
        .isEqualTo(StabilityStatus.HISTORICALLY_FLAKY);
    assertThat(failedOne.classification().type()).isEqualTo(ErrorType.AUTOMATION_FAILURE);
    assertThat(
            evaluator.evaluate(
                List.of(passed, summary("a4", ExecutionAttemptSummary.Outcome.PASSED)), 2))
        .isEqualTo(StabilityStatus.STABLE);
  }
  ...
  void incompatibleBuildAndEnvironmentDoNotCreateFlakyStatusOrRewriteCurrentResult() {
    ...
    var scope =
        new HistoricalStabilityEvaluator.CompatibilityScope(
            java.util.Set.of("build"), java.util.Set.of("local"));
  ...
  private ExecutionAttemptSummary summary(String attempt, ExecutionAttemptSummary.Outcome outcome) {
    var classification =
        outcome == ExecutionAttemptSummary.Outcome.PASSED
            ? null
