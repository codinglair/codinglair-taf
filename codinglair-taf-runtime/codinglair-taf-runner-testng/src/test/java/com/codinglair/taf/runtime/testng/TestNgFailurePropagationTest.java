...
class TestNgFailurePropagationTest {
  ...
  void jsonCarriesTheExactAuthoritativeAnalysis() {
    ...
    var attempt =
        new TestNgAttemptResult(
            1,
            Instant.EPOCH,
            TestNgAttemptResult.Status.FAILED,
            new RuntimeException("synthetic"),
            List.of(),
            analysis);
    String json =
        new TestNgStructuredResultWriter()
            .writeJson(new TestNgExecutionResult("test", "synthetic", "Example", List.of(attempt)));
    assertThat(json)
        .contains(
            analysis.classification().type().name(),
            analysis.classification().source().name(),
            analysis.signature().value(),
            analysis.signature().algorithm(),
            analysis.stability().name(),
            analysis.historyStatus().name())
        .doesNotContain("contract-canary");
  }
  ...
  void configurationFailureIsClassifiedPreservedAndReported() {
    ...
    testng.run();
    assertThat(results)
        .flatExtracting(TestNgExecutionResult::attempts)
        .anySatisfy(
            attempt -> {
              assertThat(attempt.status())
                  .isEqualTo(TestNgAttemptResult.Status.CONFIGURATION_FAILED);
              assertThat(attempt.failureAnalysis()).isNotNull();
              assertThat(attempt.failureAnalysis().classification().type())
