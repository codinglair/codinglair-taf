...
public abstract class CsvFileDataProvider<V extends IdentifiableTestData>
    extends BulkDataProvider<String, V> {
  ...
  public void initialize() {
    ...
    try (InputStream is = getClass().getClassLoader().getResourceAsStream("data/" + fileName)) {
      ...
      payload =
          allRows.stream().collect(Collectors.groupingBy(IdentifiableTestData::getTestCaseId));
    } catch (IOException e) {
